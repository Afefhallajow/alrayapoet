...
<div class="page-content">
   <div class="container-fluid">
      <!-- page title -->
      @include('admin.includes.page_title', ['title'=>'لوحة التحكم','supTitle' => 'اللقب الثاني'])
         ...
         <div class="modal-body">
            ...
            <form method="post" id="sample_form" class="form-horizontal" enctype="multipart/form-data">
               ...
               <div class="form-group row mb-3">
                  <label for="name" class="col-sm-3 col-form-label">اللقب الثاني</label>
                  <div class="col-sm-9">
               ...
               <!-- lang -->
               <div class="form-group row mb-3">
                  <label for="lang" class="col-sm-3 col-form-label">اللغة</label>
                  <div class="col-sm-9">
                     <div class="custome_blue_radio">
                        <fieldset class="radio btn-radio btn-group" data-toggle="buttons">
                           <label for="lang_ar" id="lang_for_ar" class="btn-default btn active">
                              <input type="radio" name="lang" id="lang_ar" value="ar" checked="checked">
                              <span>عربي</span>
                           </label>
                           
                           <label for="lang_en" id="lang_for_en" class="btn-default btn">
...
$(document).ready(function () {
   ...
   $('#records_table').DataTable({
      ...
      "order": [[ 0, "asc" ]],
      ajax: {
         url: "{{ route('second-surname.index') }}",
      },
      columns: [
         {
            data: 'id',
   ...
   $(document).on('click', '.edit', function () {
      ...
      $.ajax({
         url: "{{ route('second-surname.index') }}" + '/' + id + '/edit',
         dataType: "json",
         success: function (json) {
            $('#name').val(json.data.name);
            $('.custome_blue_radio label').removeClass('active');
            $('#lang_for_' + json.data.lang).addClass('active');
            $('#lang_' + json.data.lang).attr('checked', 'checked');
   ...
   $('#sample_form').on('submit', function (event) {
      ...
      $.ajax({
         url: "{{ route('second-surname.store') }}",
         beforeSend: function () {
            $('#action_button').hide();
            $('.spinner-grow').show();
         },
   ...
   $('#ok_button').click(function () {
      $.ajax({
         url: "{{ route('second-surname.index') }}" + '/' + record_id,
         beforeSend: function () {
