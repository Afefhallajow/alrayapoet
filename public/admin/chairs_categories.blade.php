...
<div class="page-content">
   <div class="container-fluid">
      <!-- page title -->
      @include('admin.includes.page_title', ['title'=>'الفعاليات','supTitle' => 'فئات الكراسي'])
      ...
      <div class="modal-content">
         ...
         <div class="modal-body">
            ...
            <form method="post" id="sample_form" class="form-horizontal" enctype="multipart/form-data">
               ...
               <!-- Text Color -->
					<div class="form-group row mb-4">
						<label for="color_text" class="col-sm-3 col-form-label">لون النص</label>
						<div class="col-sm-9" id="cp14">
							<input type="text" class="form-control colorpicker-default" name="color_text" id="color_text" />
						</div>
					</div>
               <!-- Image -->
					<div class="form-group row mb-4">
						<label for="color" class="col-sm-3 col-form-label">الصورة</label>
						<div class="col-sm-9" id="cp14">
							<input type="file" class="form-control colorpicker-default"name="image" id="image" />
						</div>
					</div>
...
$(document).ready(function () {
   ...
   $('#records_table').DataTable({
      ...
      "order": [[ 0, "asc" ]],
      ajax: {
         url: "{{ route('chairs-categories.index') }}",
      },
      columns: [
         {
            data: 'id',
         ...
         {data: 'color_text', orderable: false, searchable: false},
         {
            data: 'image',
            orderable: false,
            searchable: false,
            "render": function (data, type, row) {
               return '<img src="images/'+data+'" width="150" height="200">';
            }
         },
   ...
   $(document).on('click', '.edit', function () {
      ...
      $.ajax({
         url: "{{ route('chairs-categories.index') }}" + '/' + id + '/edit',
         dataType: "json",
   ...
   $('#sample_form').on('submit', function (event) {
      ...
      $.ajax({
         url: "{{ route('chairs-categories.store') }}",
         beforeSend: function () {
            $('#action_button').hide();
            $('.spinner-grow').show();
         },
   ...
   $('#ok_button').click(function () {
      $.ajax({
         url: "{{ route('chairs-categories.index') }}" + '/' + record_id,
         beforeSend: function () {
